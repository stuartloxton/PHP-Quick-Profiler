PHP Quick Profiler is a PHP tool to quickly debug and optimize PHP sites, the original version was released by particletree, this is a branch which adds several new features added below.

* State memory - PQP tracks it's current state and makes sure it is in the exact same state on the next page. This includes, tab, height, and which logs to show.
* Can filter log calls by type - log, error, speed and memory
* PQP is resizable via jQuery UI allowing infinite heights instead of the predefined 2 previously available.
* Selecting a tab when PQP is closed will open PQP.
* Advanced previewing of objects and arrays logged via PQP. Making it easier to use with large datasets.



h3. Original Readme below
